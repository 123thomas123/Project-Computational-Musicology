...
The song was mostly played in the C, G and  but I am not sure.
...
```{r}
bzt <- 
    get_tidy_audio_analysis('5e9TFTbltYBg2xThimr0rU') %>% # change song
    compmus_align(bars, segments) %>% # if you want to change the range of your part then change bars in three places
```


```{r}
select(bars) %>% unnest(bars) %>% 
    mutate( #cimbines pitch information
        pitches = 
            map(segments, 
                compmus_summarise, pitches, 
                method = 'rms', norm = 'euclidean')) %>% # you can change the method (rms is root mean square) to max for example, it is basically your summary statistic
     mutate( # combinbes timbre information
        timbre = 
            map(segments, 
                compmus_summarise, timbre, 
                method = 'mean')) # you can change the method, you did not need a norm in this case.
```
...
```{r}
bzt %>% 
    compmus_self_similarity(timbre, 'cosine') %>% # cosine is distance measure so you can change this to euclidean for example # you can also change timbre to pitches for a chroma gram.
    ggplot(
        aes(
            ...
            fill = d)) + 
    geom_tile() +
    coord_fixed() + # always add this line so the x axis and y axis have the same scale so you get a perfect square
...
```{r}
bluesje <- 
    get_tidy_audio_analysis('1bElCb3PUzvfpxEPCA6fmY') %>% # change song
    compmus_align(bars, segments) %>% # if you want to change the range of your part then change bars in three places
```
